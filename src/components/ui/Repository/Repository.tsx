    ...
    <>
      <div className="flex items-center gap-2">
        ...
        <div className="flex flex-col sm:flex-row flex-1 flex-wrap items-start sm:items-center sm:justify-between gap-1">
          ...
          <span className="rounded-full border border-brand/20 bg-zinc-800 text-brand px-2 py-0.5 text-xs font-medium uppercase tracking-wide ">
            {repository.language}
          </span>
        </div>
      </div>
  ...
  return (
    <ul>
      {commits.splice(0, range).map(({ node_id, commit, author }) => (
        <li
          key={node_id}
          className="relative my-2 rounded border border-zinc-800 p-2 text-sm first:border-brand/20 first:bg-brand/5"
        >
          ...
          <div className="mt-1 flex items-center gap-1">
            ...
            <p className="font-normal text-zinc-400">
              Commited by{" "}
              <Link
                className="font-medium underline sm:no-underline hover:underline hover:text-brand transition-colors"
